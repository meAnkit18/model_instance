...
**Bug #2's actual root cause, found after the "fix" above still failed differently**: pinning `jupyter-kernel-client==0.8.0` was not enough — the *same version number* behaves differently depending on where it comes from. `google-colab-cli` declares an **unversioned** dependency (`Requires-Dist: jupyter-kernel-client`, no pin at all). The public PyPI package of that name (published by Datalayer, a third party — see https://pypi.org/project/jupyter-kernel-client/) is a *different codebase* than what `google-colab-cli` actually needs, at every version checked: public 0.8.0 has `KernelClient` but not `JupyterSubprotocol`; public 1.0.2 (latest) has `JupyterSubprotocol` but not `KernelClient`. The version that actually works is **Google's own git fork**, `github.com/googlecolab/jupyter-kernel-client`, self-labeled "0.8.0" to track upstream's version number but carrying different, Google-specific code (confirmed by inspecting `direct_url.json` on the known-working local install: `vcs: git, commit: f18e982c3265df5e923aa9def101ab3fd737e139`). No version pin of the public package can ever produce this — only installing directly from that exact git commit does. Confirmed three ways before fixing: downloaded and inspected the actual PyPI 0.8.0 wheel's source (no `JupyterSubprotocol`), installed the public package under Python 3.12 and Python 3.14 (both missing one attribute or the other), then installed directly from the pinned git commit (has both). The Dockerfile now installs `jupyter-kernel-client` from that git URL/commit explicitly, before `google-colab-cli` itself, which requires `git` in the build image (added via `apt-get`).
